...
async function proxy(urlObj, reqInit, acehOld, rawLen, retryTimes, expressRes) {
  ...
  // 圧縮ヘッダーを除去（Safariの「Rawデータをデコードできません」対策）
  resHdrNew.delete('content-encoding')
  resHdrNew.delete('transfer-encoding')
...
async function httpHandler(req, pathname, expressRes) {
  ...
  // 圧縮レスポンスをそのままパイプするとブラウザがデコードできないため
  // accept-encoding を除去して非圧縮レスポンスを要求する
